...
开发时修改了其中一个模块代码，Webpack 默认会将所有模块全部重新打包编译，速度很慢。

所以需要做到修改某个模块代码，就只有这个模块代码需要重新打包编译，其他模块不变，这样打包速度就能很快。
...
上面这样写会很麻烦，所以实际开发会使用其他 loader 来解决。
...
而对 js 文件处理主要就是 eslint 、babel、Terser 三个工具，所以要提升它们的运行速度。
